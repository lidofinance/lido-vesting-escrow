...
def test_init_fail_on_zero_revokable_target(
    owner, VestingEscrowFactory, vesting_target_fully_revokable, voting_adapter
):
...
def test_init_fail_on_zero_simple_target(
    owner, VestingEscrowFactory, vesting_target_simple, voting_adapter
):
    with brownie.reverts("target_fully_revokable should not be ZERO_ADDRESS"):
        VestingEscrowFactory.deploy(
            vesting_target_simple, ZERO_ADDRESS, voting_adapter, {"from": owner}
        )
